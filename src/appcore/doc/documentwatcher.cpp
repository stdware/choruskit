...
namespace Core {
    ...
    QFileSystemWatcher *DocumentWatcherPrivate::fileWatcher() {
        if (!m_fileWatcher) {
            ...
            connect(m_fileWatcher, &QFileSystemWatcher::fileChanged, this,
                    &DocumentWatcherPrivate::changedFile);
        }
    ...
    QFileSystemWatcher *DocumentWatcherPrivate::linkWatcher() {
        ...
        if (!m_linkWatcher) {
            ...
            connect(m_linkWatcher, &QFileSystemWatcher::fileChanged, this,
                    &DocumentWatcherPrivate::changedFile);
        }
    ...
    void DocumentWatcherPrivate::addFileInfo(const QString &fileName, IDocument *document,
                                             bool isLink) {
        auto d = this;
        ...
        if (!fileName.isEmpty()) {
            ...
            d->m_documentsWithWatch[document].append(
                fileName); // inserts a new QStringList if not already there
        }
    }
    ...
    void DocumentWatcherPrivate::addFileInfo(IDocument *document) {
        ...
        const QString fixedName =
            DocumentWatcher::fixFileName(document->filePath(), DocumentWatcher::KeepLinks);
    ...
    */
    ...
    bool DocumentWatcherPrivate::saveDocument_helper(IDocument *document, const QString &fileName,
                                                     bool *isReadOnly) {
        Q_Q(DocumentWatcher);
        ...
        q->expectFileChange(
            effName); // This only matters to other IDocuments which refer to this file
        bool addWatcher =
            q->removeDocument(document); // So that our own IDocument gets no notification at all
        ...
        if (!document->save(fileName)) {
...

            {
                ...
                QMessageBox::critical(
                    ...
                    errorString.isEmpty()
                        ? QApplication::translate("Core::DocumentWatcher", "Cannot save %1")
                              .arg(filePath)
                        : QApplication::translate("Core::DocumentWatcher",
                                                  "Error while saving %1: %2")
    ...
    void DocumentWatcherPrivate::errorOnOverwrite(const QString &fileName, QWidget *parent) {
        QMessageBox::critical(
            parent, QApplication::translate("Core::DocumentWatcher", "File Error"),
    ...
    void DocumentWatcherPrivate::checkForNewFileName() {
        ...
        if (!document || document == d->m_blockedIDocument ||
            d->m_documentsWithWatch.contains(document))
            return;
    ...
    void DocumentWatcherPrivate::checkForReload() {
        ...
        if (QApplication::activeModalWidget() || d->m_blockActivated) {
            // We do not want to prompt for modified file if we currently have some modal dialog
            // open. If d->m_blockActivated is true, then it means that the event processing of
            // either the file modified dialog, or of loading large files, has delivered a file
            // change event from a watcher *and* the timer triggered. We may never end up here in a
            // nested way, so recheck later.
        ...
        PromptHandler::FileDeletedPromptAnswer previousDeletedAnswer =
            PromptHandler::FileDeletedSave;
        ...
        foreach (const QString &fileName, d->m_expectedFileNames) {
            const QString fixedName =
                DocumentWatcher::fixFileName(fileName, DocumentWatcher::KeepLinks);
            expectedFileNames.insert(fixedName);
            const QString fixedResolvedName =
                DocumentWatcher::fixFileName(fileName, DocumentWatcher::ResolveLinks);
            if (fixedName != fixedResolvedName)
        ...
        foreach (IDocument *document, changedIDocuments) {
            ...
            foreach (const QString &fileName, d->m_documentsWithWatch.value(document)) {
                ...
                FileStateItem lastState =
                    d->m_states.value(fileName).lastUpdatedState.value(document);
            ...
            // doesn't matter, because in that case we then reload the new version of the file
            // already anyhow.
            removeFileInfo(document);
            addFileInfo(document);
            ...
            if (type == IDocument::TypePermissions) {
            ...
            } else if (defaultBehavior == DocumentWatcher::ReloadUnmodified &&
                ...
                if (document->reloadBehavior(trigger, type) == IDocument::BehaviorSilent) {
                    ...
                    if (previousReloadAnswer == PromptHandler::ReloadNone) {
                        ...
                        previousReloadAnswer = promptHandler->reloadPrompt(
                            document->filePath(), document->isModified(), document->dialogParent());
                        switch (previousReloadAnswer) {
                            case PromptHandler::ReloadAll:
                            case PromptHandler::ReloadCurrent:
                                success = document->reload(IDocument::FlagReload,
                                                           IDocument::TypeContents);
                                break;
                            case PromptHandler::ReloadSkipCurrent:
                            case PromptHandler::ReloadNone:
                                success = document->reload(IDocument::FlagIgnore,
                                                           IDocument::TypeContents);
                                break;
            ...
            if (!success) {
                ...
                QMessageBox::critical(
                    ...
                    QApplication::translate("Core::DocumentWatcher", "File Error"),
                    errorString.isEmpty()
                        ? QApplication::translate("Core::DocumentWatcher", "Cannot reload %1")
                              .arg(filePath)
                        : QApplication::translate("Core::DocumentWatcher",
                                                  "Error while reloading %1: %2")
                              .arg(filePath, errorString));
            }
    ...
    DocumentWatcher::DocumentWatcher(QObject *parent)
        : DocumentWatcher(*new DocumentWatcherPrivate(), parent) {
    }
    ...
    void DocumentWatcher::addDocuments(const QList<IDocument *> &documents, bool addWatcher) {
        ...
        if (!addWatcher) {
            ...
            foreach (IDocument *document, documents) {
                if (document && !d->m_documentsWithoutWatch.contains(document)) {
                    connect(document, &QObject::destroyed, d,
                            &DocumentWatcherPrivate::documentDestroyed);
                    connect(document, &IDocument::filePathChanged, d,
                            &DocumentWatcherPrivate::filePathChanged);
        ...
        foreach (IDocument *document, documents) {
            if (document && !d->m_documentsWithWatch.contains(document)) {
                connect(document, &IDocument::changed, d,
                        &DocumentWatcherPrivate::checkForNewFileName);
                connect(document, &QObject::destroyed, d,
                        &DocumentWatcherPrivate::documentDestroyed);
                connect(document, &IDocument::filePathChanged, d,
                        &DocumentWatcherPrivate::filePathChanged);
    ...
    bool DocumentWatcher::removeDocument(IDocument *document) {
        ...
        if (!d->m_documentsWithoutWatch.removeOne(document)) {
            ...
            disconnect(document, &IDocument::changed, d,
                       &DocumentWatcherPrivate::checkForNewFileName);
        }
    ...
    bool DocumentWatcher::hasDocument(IDocument *document) const {
        Q_D(const DocumentWatcher);
        return d->m_documentsWithWatch.contains(document) ||
               d->m_documentsWithoutWatch.contains(document);
    }
    ...
    bool DocumentWatcher::saveDocument(IDocument *document, const QString &fileName,
                                       bool *isReadOnly) {
        Q_D(DocumentWatcher);
    ...
    QString DocumentWatcher::fixFileName(const QString &fileName,
                                         DocumentWatcher::FixMode fixmode) {
        QString s = fileName;
        QFileInfo fi(s);
        if (fi.exists()) {
    ...
    QString DocumentWatcher::getSaveAsFileName(const IDocument *document, const QString &path,
                                               QWidget *parent) const {
        return QFileDialog::getSaveFileName(
            parent, QCoreApplication::translate("Core::DocumentWatcher", "Save As File"), path,
            QString("%1(%2)").arg(QApplication::translate("Core::PromptHandler", "All Files"),
                                  allFilesFilter()));
    }
    ...
    DocumentWatcher::DocumentWatcher(DocumentWatcherPrivate &d, QObject *parent)
    ...
    PromptHandler::ReloadPromptAnswer PromptHandler::reloadPrompt(const QString &fileName,
                                                                  bool modified, QWidget *parent) {
        ...
        if (modified) {
            prompt = QCoreApplication::translate(
                "Core::PromptHandler", "The unsaved file %1 has changed outside. "
        ...
        msgbox.setStandardButtons(QMessageBox::Yes | QMessageBox::YesToAll | QMessageBox::Close |
    ...
    PromptHandler::FileDeletedPromptAnswer PromptHandler::fileDeletedPrompt(const QString &fileName,
        ...
        const QString title =
            QCoreApplication::translate("Core::PromptHandler", "File has been removed");
        QString msg;
        ...
        QPushButton *close = box.addButton(
            QCoreApplication::translate("Core::PromptHandler", "&Close"), QMessageBox::RejectRole);
        QPushButton *closeAll =
            box.addButton(QCoreApplication::translate("Core::PromptHandler", "C&lose All"),
                          QMessageBox::RejectRole);
        QPushButton *saveas =
            box.addButton(QCoreApplication::translate("Core::PromptHandler", "Save &as..."),
                          QMessageBox::ActionRole);
        QPushButton *save = box.addButton(
            QCoreApplication::translate("Core::PromptHandler", "&Save"), QMessageBox::AcceptRole);
